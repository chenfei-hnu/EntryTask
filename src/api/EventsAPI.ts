...
export function getEvents(
    ...
    return API.get<Events>(
        `/events?after=${after}&before=${before}&${limit(
            20,
            page
        )}&channels=${channels}`
    );
...
export function getEventsByLike(uid: number, page: number) {
    return API.get<Events>(`/user/${uid}/events?type=liked&${limit(20, page)}`);
}
//获取当前用户参与的活动列表
export function getEventsByGoing(uid: number, page: number) {
    return API.get<Events>(`/user/${uid}/events?type=going&${limit(20, page)}`);
}
//获取当前用户浏览过的活动列表
export function getEventsByPast(uid: number, page: number) {
    return API.get<Events>(`/user/${uid}/events?type=past&${limit(20, page)}`);
}

...
import { useEventListener, debounced, langFormat } from '@Utils';
...
export default function EventList({ isSettingEvents }: EventListProps) {
    ...
    const [totalEvent, setTotalEvent] = React.useState<number>(
        Number.MAX_VALUE
    ); //当前列表的最大数量，达到最大数量后不再请求后面分页的数据

    const [inited, setInited] = React.useState<boolean>(false); //初次渲染过后，search中条件变化触发响应的useEffect更新数据
    const [searchDescVisibile, setSearchDescVisibile] = React.useState<boolean>(
        false
    ); //search中条件变化后显示顶部条件描述
    ...
    const settingUserInfoHeight = isSettingEvents
        ? document.documentElement.clientHeight * 0.6
        : 0; //设置页面用户信息高度
    const topTotalHeight =
        searchDescHeight + settingUserInfoHeight + headerHeight; //无限列表顶部其他元素高度
    ...
    const scrollListen = () => {
        const index = Math.floor(
            (window.scrollY - topTotalHeight) / eventItemHeight
        );
    ...
    React.useEffect(() => {
        async function fetchUserEvents() {
            ...
            try {
                const payload = await loadUserEvents(
                    (user && user.id) || -1,
                    selectedTab,
                    page
                );
    ...
    const searchDescEl = !isSettingEvents ? (
        <div className={`eventList-searchDesc`}>
            <div className={`eventList-searchTop`}>
                ...
                >
                    {' '}
                    {`${langFormat(langType, 'CLEAR SEARCH')}`}
                </button>
            </div>
    ...
    return (
        <React.Fragment>
            ...
            <div className={`eventList-eventListContainer`}>
                ...
                {loading ? (
                    <div className={`eventList-loading`}>{`${langFormat(
                        langType,
                        'Loading...'
                    )}`}</div>
                ) : null}

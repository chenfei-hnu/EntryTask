...
export default function TabList({ data, isDetail }: TabsListProps) {
    ...
    if (isDetail) {
        ...
        tabs = data.map((tab) => (
            <DetailTab
                ...
                isSelected={
                    (detailSelectedTab &&
                        detailSelectedTab.type === tab.type) ||
                    false
                }
                onClick={() => {
                ...
                flex={tab.flex}
                icon={
                    detailSelectedTab && detailSelectedTab.type === tab.type
                        ? `icon-${tab.icon}`
                        : `icon-${tab.icon}-outline`
                }
            />
        ));
    } else {
        ...
        tabs = data.map((tab) => (
            <Tab
                key={tab.type}
                isSelected={
                    (selectedTab && selectedTab.type === tab.type) || false
                }
                onClick={() => dispatch({ type: 'SET_TAB', tab })}
                text={`${langFormat(langType, tab.type)}`}
                icon={
                    selectedTab && selectedTab.type === tab.type
                        ? `icon-${tab.icon}`
                        : `icon-${tab.icon}-outline`
                }
                count={tab.count}
    ...
    return <div className={`tabList-itemContainer`}>{tabs}</div>;
}
...
function DetailTab({ isSelected, onClick, text, icon, flex }: TabProps) {
    return (
        <div
            className={`tabList-middleItem ${
                isSelected ? `tabList-active` : ''
            }`}
            style={{ flex }}
            onClick={onClick}
        >

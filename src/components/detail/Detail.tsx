...
import { getEventDetail, getEventLikes, getEventGoings, getEventComments, likeEvent, joinEvent, commentEvent } from '@api/EventDetailAPI';
import TabList from '@common/TabList';
import { ITab } from '@reducers/eventList';
import { sliceDate, dateFormat, langFormat, getTimeDistance, useEventListener } from '@Utils';
...
export default function Detail({
  ...
  //活动详情相关的数据及状态信息
  const {
    state: { event, eventLoading, likedUsers, likedUsersLoading, goingUsers, goingUsersLoading, comments, commentsLoading, showComment },
    dispatch,
  } = useEventDetail();
  //系统当前语言
  const {
  ...
  const tabsData: ITab[] = [
    ...
    { type: 'Participants', icon: 'people', flex: langType === 'en' ? 1.3 : 1 },
  ...
  React.useEffect(() => {
    async function getDetail() {
      ...
      try {
        ...
        dispatch({ type: 'FETCH_EVENT_SUCCESS', payload: payload.data });
      } catch (error) {
    ...
    async function getLikes() {
      ...
      try {
        ...
        dispatch({ type: 'FETCH_LIKES_SUCCESS', payload: payload.data });
      } catch (error) {
    ...
    async function getGoing() {
      ...
      try {
        ...
        dispatch({ type: 'FETCH_GOING_SUCCESS', payload: payload.data });
      } catch (error) {
  ...
  return (
    <div className={`detail-page detail-bg`}  >
      {
        eventLoading || likedUsersLoading || goingUsersLoading || commentsLoading ? <div className={`detail-loading`}>Loading...</div> : null
      }
        ...
        <div className={`detail-userRow`} id='Details'>
          ...
          <div className={`detail-userDetail`} >
            ...
            <div className={`detail-pulishTime`} >
              ...
              {event && langFormat(langType, '{0}publishe', [getTimeDistance(new Date(event.create_time), langType)])}
            </div>
          </div>
        </div>
      </div>
      <div className={`${fixTabs ? `detail-fixTabs` : ''} ${langType === 'en' ? `detail-tabs` : `detail-tabsZh`}`} >
        <TabList data={tabsData} isDetail={true} ></TabList>
      </div>
      <div className={`detail-detail`} >
        {
          event && event.images.length ? (<div className={`detail-imgContainer`} >
            <img
              loading={'lazy'}
            ...
            <div className={`detail-timeLeft`} >
              <div className={`detail-timeRangeContainer`}>
                <div className={`detail-itemIcon`}>
                  ...
                  <div className={`detail-timeText`}>{event && sliceDate(event.begin_time, true)}</div>
                </div>
                <div className={`detail-itemDetail`}>{dateFormat('HH:MM', new Date(event && event.begin_time || 0))}<span className={`detail-ampm`}>am</span></div>
              </div>
            </div>
            <div className={`detail-timeRight`} >
              <div className={`detail-timeRangeContainer`}>
                <div className={`detail-itemIcon`}>
                  ...
                  <div className={`detail-timeText`}>{event && sliceDate(event.end_time, true)}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`detail-where`} id='Participants'>
          ...
          <div className={`detail-location`} >{event && event.location}</div>
          <div className={`detail-locationDetail`} >{event && event.location_detail}</div>
          <div className={`detail-locationImg`} ></div>
        </div>
      </div>
      <div className={`detail-goingLike`} id='Commens'>
        <UsersList icon={'icon-check-outline'} text={'going'} count={event && event.goings_count || 0} users={goingUsers}></UsersList>
        <div className={`detail-rowLine`} ></div>
        <UsersList icon={'icon-like-outline'} text={'likes'} count={event && event.likes_count || 0} users={likedUsers} ></UsersList>
      </div>
          ...
          <div className={`detail-commentContainer`} >
            <div className={`detail-form`} >
              ...
              <input
                ...
                value={curComment}
                placeholder={`${langFormat(langType, 'Leave your comment here')}`}
                onChange={(event) => setCurComment(event.target.value)}
              />
            </div>
            <div className={`detail-commentButton`} onClick={doComment} >
            ...
            <div className={`detail-buttonsContainer`} >
              <div className={`detail-operateLeft`} >
                <div className={`detail-operateComment`} onClick={showCommentArea}>
                  <i className={`icon-comment-single`}></i>
                </div>
                <div className={`detail-operateLike`} onClick={doLike}>
                  {
                    event && event.me_likes ? (
                      <i className={`icon-like detail-active`}></i>
                    ) : (
                ...
                >
                  {
                    event && event.me_going ? (
                      <div className={`detail-icon detail-active`}>
                        <i className={`icon-check`}>{`${langFormat(langType, 'joined')}`}</i>
                      </div>
                    ) : (
                        <div className={`detail-icon`}>
                          <i className={`icon-check-outline`}>{`${langFormat(langType, 'join')}`}</i>
                        </div>)
                  }

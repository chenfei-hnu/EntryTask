...
export default function Login(_: RouteComponentProps) {
  ...
  //页面初始是否是否显示token已失效
  const initError = !user && isAuthenticated ? langFormat(langType, 'invalid token') : null;
  ...
  return (
    <div className={`login-bg`} >
      <div className={`test-hide`} >
        {`username:${username};password:${password};loading:${loading};isAuthenticated:${isAuthenticated};langType:${langType};user.id:${user && user.id};`}
      </div>
      <div className={`login-loginContent`} >
        <div className={`login-contentBg`}>
          <div className="col-md-6 offset-md-3 col-xs-12">
            <div className={`text-xs-center login-title`} >{langFormat(langType, 'slogan')}</div>
            ...
            <form className={`login-form`} >
              ...
              <div className={`login-formItem login-marginBottom`}>
                <input
                  name="username"
                  className="form-control form-control-lg username"
                  type="username"
                  value={username}
                  placeholder={langFormat(langType, 'usernamePlaceholder')}
              ...
              <div className={`login-formItem`}>
                <input
                  ...
                  type="password"
                  value={password}
                  placeholder={langFormat(langType, 'passwordPlaceholder')}
                  onChange={(event) => setPassword(event.target.value)}
                />
                <i className={`icon-password`}></i>
              </div>
              <div className={`login-formItem login-error`}>
                {errors || initError && <ListErrors errors={errors || initError} />}
              </div>

...
export default function Setting(_: RouteComponentProps) {
  ...
  //设置页面我的相关活动信息tabs项数据
  const tabsData: ITab[] = [
    { type: 'Likes', icon: 'like', count: user && user.likes_count || 0, flex: 1 },
    { type: 'Going', icon: 'check', count: user && user.goings_count || 0, flex: 1 },
    { type: 'Past', icon: 'past', count: user && user.past_count || 0, flex: 1 },
  ];
    ...
    <div className="settings-page">
      <div className="page-content">
        <div className={`setting-topContainer`}>
          ...
          <div className={`setting-username`}>{user && user.username}</div>

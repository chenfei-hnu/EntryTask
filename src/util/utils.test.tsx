...
describe('Testing the utils', () => {
    ...
    test('Test getDate', () => {
        ...
        expect(
            Math.ceil((tomorrow - new Date().getTime()) / (1000 * 60 * 60 * 24))
        ).toBe(1);
    });

    test('Test debounced', async () => {
        const test = jest.fn();
        const debouncedHandler = debounced(test, 200);
    ...
    test('Test getTimeDistance', () => {
        ...
        const minutesAgoResult = getTimeDistance(
            new Date(now - 1000 * 61),
            'en'
        );
        expect(minutesAgoResult).toBe('1 minutes ago');

        const hoursAgoResult = getTimeDistance(
            new Date(now - 1000 * 61 * 60),
            'en'
        );
        expect(hoursAgoResult).toBe('1 hours ago');

        const yestodayResult = getTimeDistance(
            new Date(now - 1000 * 61 * 60 * 24),
            'en'
        );
        expect(yestodayResult).toBe('Yestoday');

        const daysAgoResult = getTimeDistance(
            new Date(now - 1000 * 61 * 60 * 24 * 2),
            'en'
        );
